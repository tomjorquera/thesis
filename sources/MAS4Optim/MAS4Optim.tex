...
\subsection{Agents Behavior and Co-design}
...
Let $M(I,O)$ be a model agent with a set of inputs $I$ and a set of outputs $O$.

Let $\{r = <\Delta_{o_{\in O}}^{r}, crit>\}$ be a set of requests received for outputs $\{o \in O\}$, where $\Delta_{o}^{r}$ is the variation of output $o$ requested by the request $r$ and $crit$ the criticality of the request. By hypothesis, at most one request can concern the same output.\\
...
Otherwise, the model agent will discriminate using the relative participation of the input on the outputs concerned by the requests, calculated as follows: 
$\Delta_i^* \leftarrow \left\{ \Delta_i^{r_{max}} : r_{max} = argmax \dfrac{part_{i,o_{r_{max}}}}{\displaystyle\sum_{i \in I} part_{i,o}} \right\}$
That is we select variation from the requests for which the relative participation of the input (in regard of the total participation) is the highest.

If at this step $|\Delta_i^*| = 1$, the unique $\Delta_i^*$ is selected. If several conflicting requests have an equal maximal participation, they are considered equally important and one of them is selected randomly[[True?]].
...
Using this new formulation, no model agent is create during the MAS transformation of the problem, but the underlying problem between cooperative trajectories still exists but with an additional difficulty. As we already said, in order to be able to scale with the complexity of the problem, each agent must keep a local view. Consequently, the criteria agents are not aware of each other, and neither are the variable agents.\\
