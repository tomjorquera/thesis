...
[[TODO How is this value propagated regarding cooperative trajectories?]]
...
In this section we propose a somewhat middle ground mechanism, where the agents try to learn and anticipate the messages they expect to receive for taking their decision. Using this mechanism, an adaptation period will be necessary for the system before all the agents have correctly learned the messages they should wait, during which the agents will have a suboptimal behavior, as NCS may not be correctly detected. The iterative nature of the solving process allows these suboptimal decisions to be ultimately corrected by the agents themselves.

[[TODO: compete with the textual description + the algorithm of the waiting mechanisms]]

[[Mail de Victor:

Donc, pour ta requête, je crois que c'est assez simple, si je ne me trompe pas, l'idée est que les agents n'agissent pas tant qu'ils ont pas :
1) toute l'info en terme de requêtes : donc ils attendent UNE requête par sender par output (donc 1 par output pour les modèles, autant qu'ils connaissent de sender pour les variables)
2) un certain pourcentage d'info (par rapport aux corrélations) pour les informs ( par input pour les variables).

Mais si ils n'ont pas envoyé d'inform sur un output, alors il n'attendent pas de requêtes
Si ils n'ont pas envoyé de requêtes sur un input, alors ils considèrent qu'ils ont déjà l'info.

Ensuite, si ils reçoivent un inform avec le flag trucmuche, alors ils le passent... ou alors peut-etre pas, je m'en rappelle plus ça, mais le code devrait être explicite je pense... (dans inform handling, mais peut-être vérifie tout les utilisateurs de cette info dans eclipse...).
]]
...
\begin{tabular}{|c|c|p{5cm}|p{3.5cm}|}
	...
	\hline
	Hidden Dependencies & [[?]] & [[?]] & origin timestamps, influence\\
	\hline
	Asynchronous Requests & [[?]] & Agents receive messages in untimely manner & [[?]]\\
	\hline
\end{tabular}
...
\begin{tabular}{|c|p{3.5cm}|p{3.5cm}|p{5cm}|}
	...
	\hline
	Sender/Origin Timestamps & A unique signature composed of the unique id of the emitter/origin of the message and a (local) time step & Cycles Solving, Hidden Dependencies, Asynchronous Requests, [[Cooperative Trajectories ?]] & Comparable by sender/origin, allows a partial ordering of the messages (by sender/origin)\\
	\hline
...
\subsection{Complexity Analysis of Solving Mechanisms}

In this section, we provide a quick complexity analysis of the different solving mechanisms.
...
[[say how these NCS can be generalized to problems others than ID4CS => cue to CPSP]]
