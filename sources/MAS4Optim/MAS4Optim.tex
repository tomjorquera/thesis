...
Any uncertainty representation which can provide these four informations can be used by the system. All the designer need to do is to provide the module which implements these operations\footnote{The reader familiar the concepts of object-oriented programming and interface implementation will have no difficulties to guess how the process is done in our prototype}.

Let us see how different representations can provide these information with two drastically different modelings: intervals and Monte Carlo experiments.

\subsection{Intervals Representation}

As the reader knows, an interval representation consist of a lower and upper boundaries, with all the values between the boundaries considered as equally likely. Suppose $l$ and $u$ the boundaries of our uncertainty.
...
\item the cumulative probability of $t$ is calculated as $P(X < t) =
			\begin{cases}
				...
				0	&\text{ otherwise}\\
			\end{cases}$
\item the inverse cumulative probability of $p$ is obtained as $ t = p (u - l)$.
\end{compactitem}
...
\begin{compactitem}
\item the mean can be obtained as $\dfrac{\sum_n{x_n}}{n}}$.
...
\item the cumulative probability of $t$ is calculated as  $P(X < t) = \dfrac{|\{x_i, \forall x_i < t\}|}{|\{x_n\}|}$ (where $|{x|$ is the cardinality of the set ${x}$), the number of elements lower than $t$ divided by the total number of elements.
\item the inverse cumulative probability of $p$ is obtained as $ t = $.
\end{compactitem}
...
\subsection{Examples of Propagator}
