...
Output agents have more \enquote{passive} role than variable agents during problem solving. As it depends on a model agent for its value, an output agent will simply transmits the requests it receives to its model, as summarized in algorithm \ref{algo_solving_output}.
...
\label{algo_criticality_anticipation}
	...
	$R_{equiv} \leftarrow r \in R-\{r_{max}\} \text{ such as }\begin{cases}
