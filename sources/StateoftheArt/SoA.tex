\part{Context of the Study and State of the Art}
...
\definition{Search space}{the set containing all the possible candidate solutions for the optimization problem.}
...
\definition{Objective-function}{a function defined over the search space of the optimization problem, expressing the goal of a stakeholder for this problem.}
...
One example which can be connected to the NFL is the compromise between exploration and exploitation. Basically, an optimization method must often make a compromise between using the previous results to converge inside a region of the search space and exploring the rest of the search space to find a better region.
...
On a side note, it can be added that, in addition to the case of continuous optimization, the possible existence of free lunches (\emph{i.e.} there exist optimization methods strictly better than some others) has been demonstrated for coevolutionary \cite{1545946} as well as multi-objective \cite{1299403} optimization problems.
...
A rough proposal of organization of the different types of continuous optimization can be seen on \figurename{} \ref{numerical_optim_tree}.
...
\caption{PSO iteration}\label{PSO_algo}
	...
	\ForEach{particle}{
		$r_l, r_g \leftarrow$ random numbers $\in [0,1]$\;
		$\overrightarrow{v_i}(t) = w  \overrightarrow{v_i}(t-1) + c_l  r_l  (x_i^*(t-1)-x_i(t-1)) + c_g  r_g (x_g^*(t-1)-x_i(t-1))$\;
...
In the case where the designer would have enough knowledge to meaningfully use such methods, the introduction of this knowledge may introduce a bias in the resulting solution, orienting the optimization process toward \enquote{standard} solutions at the expense of possible non-conventional ones.
...
\definition{Pareto-dominance}{Given A and B two vectors describing resources allocations to different individuals in a system, \(A \prec B \Leftrightarrow (\forall i \text{ }A_i \leq B_i \land \exists j \text{ } A_j < B_j\)). }
...
It has also be noted in \cite{corne2007techniques} that many MOEA have poor scalability performances in regard of the number of objectives, often having troubles to handle more than five objectives. This degradation of performances is attributed to the combined factors of:
\begin{compactitem}
...
%
%To illustrate the difficulties caused by coupled system, let us introduce the Fixed Point problem.
%
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{mdo_mdf}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_idf}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_aao}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_csso}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_co}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_bliss}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_aso}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_mdois}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{mdo_qsd}
