...
One a side note, it is interesting to notice how many methods requires non-trivial changes to topology of the agent graph to apply (acyclic graph, tree structure ...), both in the context of DCSP and DCOP. These changes can potentially be extensive operations in themselves, and must be applied carefully lest the relevance of the result be compromised. Consequently, existing agent-based optimization techniques for DCOP can require a strong expertise to be efficiently applied\cite{Ka2011.6}.
