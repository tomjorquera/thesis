...
As stated at the end of the last chapter, providing a method able to scale to the needs of the full range of optimization problems would require it to be capable of adapting itself to the problem at hand.
...
ADELFE \cite{bernon2003adelfe} is a method dedicated to the development of AMAS. The name \enquote{ADELFE} is the French acronym for "toolkit to develop software with emergent functionality" (\textit{Atelier pour le DEveloppement de Logiciels à Fonctionnalité Emergente}). While ADELFE is not the only method devoted to guide the design of a MAS, its specificity concerns the fact that it is specifically tailored for AMAS.
