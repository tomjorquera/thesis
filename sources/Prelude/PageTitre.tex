...
\CompositionDuJury{
  ...
  \begin{tabular}{lll}
    \multicolumn{3}{c}{\textsc{Jury}\vspace{0.5em}}\\
    	Abderrafiaa \textsc{Koukam} & \emph{Professor, Université de Technologie de Belfort-Montbéliard[[TL;DF]]} & (Reviewer)\\
		René \textsc{Mandiau} & \emph{Professor, Université de Valenciennes} & (Reviewer)\\
		\\
		Juan A. \textsc{Rodríguez-Aguilar} & \emph{[[??]]} & (Examiner)\\
		Gauthier \textsc{Picard} & \emph{Associate Professor, ENS[[?]] des Mines de Saint-\'Etienne} & (Reviewer)\\
    	\\
	    Marie-Pierre \textsc{Gleizes} & \emph{Professor, Université de Toulouse III} & (Supervisor)\\
  		Jean-Pierre \textsc{Georgé} & \emph{Associate Professor, Université de Toulouse III} & (Co-Supervisor)\\	
    	& & \\
  \end{tabular}
  \end{center}
}
