...
\item For each spring linked to at least one free node, a model agent is created, representing the current tension of the spring based on its initial tension and the coordinates of the nodes to which the spring is linked. The exact computations for calculating the tension strength of a spring is $(x_1 - x_2) \times k$, where $x_1, x_2$ are the coordinates of the linked nodes and $k$ is the initial tension of the spring. This model takes in input the value of the variable agents of the linked nodes (if one of the node is fixed, its coordinate is used as a constant). For each model agent, an output agent is created representing the output value of the model.
