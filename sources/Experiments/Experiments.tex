...
The \emph{Simplicity} criterion concerns the amount of work required to instantiate the method to suit different optimization problem. The original authors chose to evaluate this criterion using to measures: the number of optimizers involved and the number of additional variable required by the method.
...
\text{s.t. }	&	g1 = \dfrac{y_1}{3.16} - 1 \geq 0\\ 
				&	g2 = 1 - \dfrac{y_2}{24} \geq 0\\
...
The authors use the same initial points $x_1 = 1$, $x_2 = 5$, $x_3 = 2$ for each analysis. The optimum for this problem is located at $x_1 = 1.9776$, $x_2 = 0$, $x_3 = 0$, for which the value of $f$ is $3.1834$.
...
\begin{table}
\caption{Simplicity}\label{bench1_simplicity}
\centering
\begin{tabular}{ccc}
\toprule
...
\begin{table}
\caption{Efficiency}\label{bench1_efficiency}
\centering
\begin{tabular}{cccc}
\toprule
...
\begin{table}
\caption{Accuracy}\label{bench1_accuracy}
\centering
...
Here we present a synthesis of the comparison between our system and existing MDO methods. the only method dominating our own in regard of this criterion is MDF. Our method can be considered on part with IDF, as neither of them dominate the other on both measures.
...
\begin{tabular}{cccccc}
		...
		\midrule
		best & \textbf{AMAS} & \textbf{AMAS} & \textbf{AMAS} & MDF & \textbf{AMAS}\\
		\mknode{n1}	& MDF & IDF & MDF & IDF, \textbf{AMAS} & CO\\
			& IDF & BLISS & IDF &  & CSSO\\
...
In regard of the accuracy criterion, [[TODO]].

In regard of the efficiency criterion, [[TODO]].

In regard of the simplicity criterion, for the number of objectives, our method requires $n$ optimizers ($n$ being the number of models involved). In this regard it is less efficient than MDF and IDF, which only require 1 optimizer, but is better than the other methods which require $n+1$ optimizers (1 by model plus 1 global optimizer). For the number of additional variables, our method in on part with MDF, the best method in regard of this measure, as neither of them require any additional variable.
...
In regard of the portability criterion, our method is extremely flexible as it support multiple levels of modeling and multiple reformulations of the problem. Consequently the problem can be \enquote{tailored} to fit the existing organizational structure. Contrary to the MDO methods, our method does not mandate a \emph{central} authority as the responsibility to maintaining consistency is distributed in the system.
